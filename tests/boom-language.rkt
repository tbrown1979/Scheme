...
(define boom/apply-op
  (lambda (operator first second)
    (cond ((eq? operator '+) +)
          ((eq? operator '-) -)
          ((eq? operator '*) *)
          ((eq? operator '/) (if (and (integer? first) (integer? second))
...
(define boom/eval
  (lambda (exp)
    (boom/eval-exp (boom/preprocess exp)  base-env)))
